...
static int generateCode(const char *key, unsigned long tm) {
  ...
  HMAC(EVP_sha1(),secret,secretLen, challenge,8,hash, &dlen);


  // Pick the offset where to sample our hash value for the actual verification
  // code.
  int offset = hash[dlen - 1] & 0xF;
...
static const char *getUserName(uid_t uid) {
  ...
  if (getpwuid_r(uid, &pwbuf, buf, len, &pw) || !pw) {
    user = reinterpret_cast<char *>(malloc(32));
...
static const char *getURL(const char *secret, char **encoderURL) {
  ...
  char *url = reinterpret_cast<char *>(malloc(strlen(user) + strlen(hostname) + strlen(secret) + 80));
  sprintf(url, "otpauth://totp/%s@%s?secret=%s", user, hostname, secret);
  if (encoderURL) {
    const char *encoder = "https://www.google.com/chart?chs=200x200&"
                          "chld=M|0&cht=qr&chl=";
    *encoderURL = reinterpret_cast<char *>(malloc(strlen(encoder) + strlen(url) + 6));
...
int main(int argc, char *argv[]) {
  ...
  if (ch == 'y' || ch == 'Y') {
    ...
    char *fn = reinterpret_cast<char *>(calloc(1, 2*(strlen(home) + strlen(SECRET) + 2)));
    if (!fn) {
